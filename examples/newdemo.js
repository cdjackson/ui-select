...
var app = angular.module('demo', ['ngSanitize', 'ui.select', 'ui.select.sort', 'ui.select.tagging']);
...
app.controller('DemoCtrl', function ($scope, $http, $timeout, $interval) {
    ...
    var tagging = {isActivated: false, fct: undefined};
    var taggingTokens = {isActivated: false, tokens: undefined};
    ...
    $scope.onBeforeSelect = function ($item) {
        var $select = this.$select;

        // For tagging, use the search if there's no item selected
        var item = $item !== undefined ? $item : $select.search;

        return !_findCaseInsensitiveDupe(item, $select.selected);
    };


    function _findCaseInsensitiveDupe(search, arr) {
        if (arr === undefined || search === undefined) {
            return false;
        }
        var hasDupe = arr.filter(function (origItem) {
                if (search.toUpperCase() === undefined || origItem === undefined) {
                    return false;
                }
                return origItem.toUpperCase() === search.toUpperCase();
            }).length > 0;
    ...
    function _findApproxDupe(haystack, needle) {
        ...
        if (angular.isArray(haystack)) {
            for (var i = 0; i < haystack.length; i++) {
                if(needle.indexOf(haystack[i]) == 0) {
                    dupeIndex = i;
                }
            }
        }
